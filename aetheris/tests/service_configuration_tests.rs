...
#[tokio::test]
async fn test_nextcloud_configure_writes_autoconfig() {
    ...
    let secrets = Secrets {
        nextcloud_db_password: Some("secret_db_pass".to_string()),
        nextcloud_admin_password: Some("secret_admin_pass".to_string()),
        ..Secrets::default()
    };
    ...
    assert!(content.contains("secret_admin_pass"));
}

#[tokio::test]
async fn test_nextcloud_configure_skips_if_config_exists() {
    let system = SpySystem::default();
