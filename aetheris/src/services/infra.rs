...
impl Service for MariaDBService {
    ...
    async fn configure(&self, runtime: &dyn crate::ports::SystemPort, hw: &HardwareInfo, secrets: &Secrets) -> Result<()> {
...
"#, buffer_pool, log_file_size, max_connections);
    ...
    fn env_vars(&self, hw: &HardwareInfo, secrets: &Secrets) -> HashMap<String, String> {
        ...
        vars.insert("MYSQL_DATABASE".to_string(), "aetheris".to_string());
        vars.insert("MYSQL_USER".to_string(), "aetheris".to_string());
...
impl Service for HomepageService {
    ...
    fn ports(&self) -> Vec<String> { vec!["3002:3000".to_string()] }
    fn volumes(&self, _hw: &HardwareInfo) -> Vec<String> {
        vec!["./config/homepage:/app/config".to_string(), "/var/run/docker.sock:/var/run/docker.sock:ro".to_string()]
    }
    fn resources(&self, hw: &HardwareInfo) -> Option<ResourceConfig> {
        let memory_limit = match hw.profile {
            HardwareProfile::High => "512M",
