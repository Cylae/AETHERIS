...
impl Service for MariaDBService {
    ...
    async fn configure(
        &self,
        runtime: &dyn crate::ports::SystemPort,
        hw: &HardwareInfo,
        secrets: &Secrets,
    ) -> Result<()> {
        let init_dir = Path::new("./config/mariadb/initdb.d");
        runtime
            .create_dir_all(init_dir)
            .context("Failed to create mariadb initdb.d")?;
        ...
        runtime
            .write_file(&init_dir.join("init.sql"), &sql)
...
"#,
        ...
        runtime
            .write_file(&config_dir.join("custom.cnf"), &custom_cnf)
