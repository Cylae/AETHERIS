...
impl Secrets {
    pub fn load_or_create<P: AsRef<Path>>(path_arg: P) -> Result<Self> {
        let home_path = if let Ok(home) = std::env::var("AETHERIS_HOME") {
            Path::new(&home).join(path_arg.as_ref())
        } else {
            path_arg.as_ref().to_path_buf()
        };
        ...
        let mut secrets: Secrets = if path.exists() {
            ...
            serde_yaml_ng::from_str(&content).context("Failed to parse secrets file")?
        } else {
            Secrets::default()
        };
