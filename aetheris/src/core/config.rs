...
fn get_config_path() -> PathBuf {
    if let Ok(path) = std::env::var("AETHERIS_CONFIG") {
        return PathBuf::from(path);
    }
    if let Ok(home) = std::env::var("AETHERIS_HOME") {
        return Path::new(&home).join("config.yaml");
    }
...
impl Config {
    ...
    pub fn save(&self) -> Result<()> {
        ...
        let mut file = options.open(&path).context("Failed to open config file for writing")?;
        use std::io::Write;
